...
int put(hashtable* ht, keyType key, valType value){
    ...
    if (curEntry == NULL) {
        ...
        while (((curEntry->key.x != key.x) || (curEntry->key.y != key.y)) && curEntry->next != NULL) {
            curEntry = curEntry->next;
        }

        // if the key exists in the hashtable, return
        if ((curEntry->key.x == key.x) && (curEntry->key.y == key.y)) {
            return 0;
        }
...
int update(hashtable* ht, keyType key, int limb){
    ...
    if (curEntry == NULL) return -1;
        ...
        // if the key exists in the hashtable, update the value
        if ((curEntry->key.x == key.x) && (curEntry->key.y == key.y)) {
            curEntry->value += pow(2, limb);
        }

        // if the key does not exist in the hashtable, return -1
        else return -1;
    }
...
int limb_visited(hashtable* ht, node hold, int limb) {
    int value = get(ht, hold);
    int this_limb = (int) pow(2, limb) & value;
    if (this_limb == 0) return 0;
    return 1;
}
...
valType get(hashtable* ht, keyType key){
    ...
    if ((curEntry->key.x == key.x) && (curEntry->key.y == key.y)) return curEntry->value;

    // advance curEntry and nextEntry until the key matches or it reaches the end of the list
    entry* nextEntry = curEntry->next;
    while (nextEntry != NULL && ((nextEntry->key.x != key.x) || (nextEntry->key.y != key.y))) {
        curEntry = nextEntry;
