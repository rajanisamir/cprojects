...
int put(hashtable* ht, keyType key, valType value){
    ...
    if (curEntry == NULL) {
        ...
        while (strcmp(curEntry->key, key) != 0 && curEntry->next != NULL) {
...
int hash(keyType key, int m){
    int index = 0;
    // sums the ASCII codes for each character
